package fr.erased.clans.utils.commands;
...
public class CommandFramework implements CommandExecutor {
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage("Erreur: Vous devez être un joueur pour effectuer cette commande");
                    return true;
                }
                try {
    ...
    private void defaultCommand(CommandArgs args) {
        args.getSender().sendMessage(ChatColor.RED + "Erreur: Cette commande n'est pas exécutable.");
    }
